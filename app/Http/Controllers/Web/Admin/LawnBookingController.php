 ...
 * vechicle types Apis
 */
class LawnBookingController extends BaseController
{
    /**
    ...
    */
    ...
    //    dd("sdvsdfs");
        ...
        return view('admin.lawn_booking.index', compact('page', 'main_menu', 'sub_menu','sub_menu_1','user','title'))->render();
    }

    public function getAllTypes(QueryFilterContract $queryFilter,Request $request)
    { 
        // dd($request->all());
        $query = LawnBooking::where('booked_by',auth()->user()->id);
        $type = $request->type;
        switch ($type) {
            case 'pending':
        ...
        return view('admin.lawn_booking._types', compact('results','type'))->render();
    }
    ...
    */
    ...
    { 
       ...
       $date_diff = $checkinDate->day - $currentDate->day;
        ...
        // dd($daysDifference);
        $results = $queryFilter->builder($query)->customFilter(new CommonMasterFilter)->paginate();
        return view('admin.lawn_booking.view', compact('page', 'main_menu', 'sub_menu','sub_menu_1','booking','date_diff'))->render();
    }
    /**
    * Get Types by admin for ajax
    *
    */
    public function book_now(Request $request)
    {
        ...
        if(isset($request->id))
        {
            $totalDaysBooked = LawnBooking::whereBetween('checkin_date', [$startOfMonth, $endOfMonth])
    ...
    */
    public function check_availability(Request $request)
    {
        ...
        if(isset($request->id))
        {
            $bookings = LawnBooking::where(function($query) use ($request) {
            ...
            $bookings = LawnBooking::whereBetween('checkin_date', [$request->from_date, $request->to_date])->orwhereBetween('checkout_date', [$request->from_date, $request->to_date])
                                ->whereIn('status',[0,1]) 
                                ->get();
        } 
        // if(count($bookings) == 0)
    ...
    */
    ...
    {   
        ...
        $sub_menu_1 = '';
        $user = $this->user->belongsTorole(Role::USER)->get(); 
        return view('admin.lawn_booking.update', compact('page','main_menu', 'sub_menu','sub_menu_1','user','booking'));
    }
