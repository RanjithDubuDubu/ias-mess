    ...
    */
    ...
    { 
        ...
        if($request->type == "pending")
        {
            ...
            $title = 'Pending Room Booking';
        }
        if($request->type == "checkin")
        {
            $sub_menu = 'checkin-booking';
            $title = 'Checkin Room Booking';
        }
        if($request->type == "cancelled")
        {
            $sub_menu = 'cancelled-booking';
            $title = 'Cancelled Room Booking';
        }
        if($request->type == "completed")
        {
            $sub_menu = 'completed-booking';
            $title = 'Completed Room Booking';
        }
        $sub_menu_1 = '';
        $user = $this->user->belongsTorole(Role::USER)->get(); 
        return view('admin.room_booking.index', compact('page', 'main_menu', 'sub_menu','sub_menu_1','user','title'))->render();
    }
    ...
    { 
        ...
        return view('admin.room_booking._types', compact('results','type'))->render();
    }
